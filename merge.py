...
def merge(destination):
	...
	for file in os.listdir("cfg"):
		cfg_src = os.path.join("cfg", file)
		cfg_dest = os.path.join(destination, "cfg")
		print("\tCopying:\t %s" % file)
		shutil.copy2(cfg_src, cfg_dest)

	print("\nMerging '/gamemodes/terrortown/entities/weapons/' ...\n")
	for file in os.listdir("gamemodes/terrortown/entities/weapons"):
		weapons_src = os.path.join("gamemodes", "terrortown", "entities", "weapons", file)
		weapons_dest = os.path.join(destination, "gamemodes", "terrortown", "entities", "weapons")
		print("\tCopying:\t %s" % file)
		shutil.copy2(weapons_src, weapons_dest)

	print("\nMerging '/lua/weapons/' ...\n")
	for file in os.listdir("lua/weapons"):
		lua_src = os.path.join("lua", "weapons", file)
	...
	for file in os.listdir("sound/endroundmusic/innocent"):
		# innocent music
		innocent_src = os.path.join("sound", "endroundmusic", "innocent", file)
		innocent_dest = os.path.join(destination, "sound", "endroundmusic", "innocent")
		print("\tCopying:\t %s" % file)
		shutil.copy2(innocent_src, innocent_dest)

	for file in os.listdir("sound/endroundmusic/traitor"):
		# traitor music
		traitor_src = os.path.join("sound", "endroundmusic", "traitor", file)
